...
fn make_action_step(name: &str, kind: &str) -> Step {
    Step {
        id: "1".into(),
...
fn resolve_remote_repository() {
    let mut step = make_action_step("actions/checkout", "repository");
...
fn resolve_remote_with_path() {
    let mut step = make_action_step("actions/aws", "repository");
...
fn resolve_local_self_repository() {
    let mut step = make_action_step(".github/actions/my-action", "repository");
...
fn resolve_docker_container_registry() {
    let mut step = make_action_step("docker://node:18", "containerregistry");
...
fn resolve_fallback_name_parsing() {
    let step = make_action_step("actions/checkout@v4", "");
...
fn resolve_fallback_with_subpath() {
    let step = make_action_step("actions/aws/configure-credentials@v1", "");
